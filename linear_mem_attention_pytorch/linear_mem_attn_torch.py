...
from .utils import (
    dynamic_slice, torch_map, torch_scan
)
...
def attention(
    query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, mask: Optional[torch.Tensor] = None,
    query_chunk_size: int = 1024, key_chunk_size: int = 4096,
) -> torch.Tensor:
    ...
    def chunk_scanner(chunk_idx: int, _):
        ...
        return (
            ...
            query_chunk_attention(query_chunk, key, value, mask, key_chunk_size=key_chunk_size)
        )

    _, res = torch_scan(chunk_scanner, init=0, xs=None, length=math.ceil(num_q / query_chunk_size))
